...
class RPCVISolver(object):
    ...
    def run(
        ...
        for i in range(1, iterations):
            v_table = self._rpc_value_iteration(utility_grid=v_table, discount=discount)
            policy_grids[..., i] = self._get_best_policy(v_table, discount=discount)
            utility_grids[..., i] = v_table

            # the # of iterations for convergence
            if i <= iter_cvg:
                if np.linalg.norm(utility_grids[..., i] - utility_grids[..., i - 1], 2) <= 1e-12:
                    iter_cvg = i
                    print(f"The number of iterations to convergence for VI: {iter_cvg}")
        ...
        return result
    def _rpc_value_iteration(self, utility_grid, discount):
