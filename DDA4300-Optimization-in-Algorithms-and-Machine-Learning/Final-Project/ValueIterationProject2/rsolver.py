...
class RandomValueIterationSolver(object):
    """Random Value Iteration Solver."""
    def __init__(
            self,
            cost_matrix: np.ndarray,
            transition_matrix: np.ndarray,
    ...
    def run(
        ...
        for i in range(1, iterations):
            # print(i)
            criterion = lambda x: random() < 0.5 # add random information
            updates = list(set(filter(criterion, range(self._num_state)))) # add random information
            # print(updates)
            v_table = self._random_value_iteration(updates, utility_grid=v_table, discount=discount) # add random information
            policy_grids[..., i] = self._get_best_policy(v_table, discount=discount)
            utility_grids[..., i] = v_table
        policy_grids = policy_grids.transpose([1, 0])
        utility_grids = utility_grids.transpose([1, 0])
    ...
    # rand_vi
    def _random_value_iteration(self, updates, utility_grid, discount=1.0):
        value_out = utility_grid
        for i in updates: # updates is the random sample from the state vector
            value_out[i] = self._calculate_utility(i, discount, utility_grid)
        return value_out
